...
class ABA(Paths):
    ...
    def get_experiments_for_region(self, region, cre=False):
        """
            [Downloads and returns experiments metdata for a brain region]

            Arguments:
                region {[str]} -- [acronym of a brain region]

            Keyword Arguments:
                cre {[bool]} -- [If true results for cre lines experiments are returned.]
        """
        region = self.structure_tree.get_structures_by_acronym([region])[0]
        region_experiments = self.mcc.get_experiments(cre=cre, injection_structure_ids=[region['id']])
        print("{} experiments found for {}".format(len(region_experiments), region['name']))
        return region_experiments

    def _get_hemisphere_id(self, hemisphere):
            if hemisphere is not None:
                if hemisphere == 'both':
                    hemisphere = 3
                elif hemisphere == 'right': # right hemisphere, ipsilateral
                    hemisphere = 2
                elif hemisphere == 'left':
                    hemisphere = 1
                else:
                    raise ValueError("Unrecognize value for hemisphere argument: ['left', 'right', 'both', 'None']")
            return hemisphere

    def get_experiments_summary_for_region(self, region, cre=False, summary_structures=None, hemisphere="both"):
        """
            [Returns unionized experimental data]
            ...
            Keyword Arguments:
                cre {[bool]} -- [If true results for cre lines experiments are returned.]
                summary_structures {[list]} -- [List of integers with IDs of brain regions to unionize over. 
                    If none, the default structures are used.]
        """
        hemisphere = self._get_hemisphere_id(hemisphere)

        if summary_structures is None:
            summary_structures = list(self.structures['id'].values)

        experiments = self.get_experiments_for_region(region)

        print("Downloading experiments summary")
        structure_unionizes = self.mcc.get_structure_unionizes([ e['id'] for e in experiments ], 
                                                  is_injection=False,
                                                  structure_ids=summary_structures,
                                                  include_descendants=False)

        if hemisphere is not None:
            structure_unionizes = structure_unionizes.loc[structure_unionizes.hemisphere_id == hemisphere]

        # Select only large
        large_unionizes = structure_unionizes[structure_unionizes.volume > .5 ]
        return large_unionizes

    def get_projection_matrix_for_region(self, region, hemisphere='both', parameter='projection_density', regions_ids=None,  **kwargs):
        hemisphere = self._get_hemisphere_id(hemisphere)
        structure_unionizes = self.get_experiments_summary_for_region(region, **kwargs)


        if regions_ids is None:
            regions_ids = list(self.structures['id'].values)

        mtx =  self.mcc.get_projection_matrix(experiment_ids = list(set(structure_unionizes['experiment_id'].values)), 
                               projection_structure_ids = regions_ids,
